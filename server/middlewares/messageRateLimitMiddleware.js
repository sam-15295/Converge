...
const createChatLimiter = (limit, message, prefix)=> createUserLimiter({windowMs : 10 * 1000, limit, message, prefix});

// sending messages and replies
const messageRateLimitMiddleware = createChatLimiter(env.CHAT_RATE_LIMIT_MAX, "You are sending messages too fast. Please slow down.", "rl:message:");

// adding and removing reactions (a click is cheaper than a message, so the budget is twice as big)
export const reactionRateLimitMiddleware = createChatLimiter(env.CHAT_RATE_LIMIT_MAX * 2, "You are reacting too fast. Please slow down.", "rl:reaction:");
